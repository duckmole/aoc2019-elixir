defmodule Day9 do
  ...
  def intcode(code, child_pid, thrusters_pid) do
    ...
    intcode(List.first(decode), %{index: 0, relative_base: 0}, decode, %{child: child_pid, thrusters: thrusters_pid}, 0)
  end
  ...
  def intcode(operator, indexes, result, pids, last_output) when rem(operator,100) == 3 do
    [_, dest, _, _] = decompose(operator, indexes, result)
    receive do
      {:output, input} ->
        new_result = compute_result(input, dest, result)
        intcode(Enum.at(new_result, indexes[:index] + 2), %{indexes | index: indexes[:index] + 2}, new_result, pids, last_output)
    end
  end
  def intcode(operator, indexes, result, pids, _) when rem(operator,100) == 4 do
    ...
    intcode(Enum.at(result, indexes[:index] + 2),  %{indexes | index: indexes[:index] + 2}, result, pids, first)
  end
  ...
  def intcode(operator, indexes, result, pids, last_output) do
    ...
    intcode(Enum.at(new_result, indexes[:index] + 4), %{indexes | index: indexes[:index] + 4}, new_result, pids, last_output)
  end
